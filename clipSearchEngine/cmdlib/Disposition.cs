...
namespace cmdlib
{
    ...
    public class Tags
    {
        ...
        public string NUMBEROFFRAMESEng { get; set; }

        [JsonPropertyName("NUMBER_OF_BYTES-eng")]
        public string NUMBEROFBYTESEng { get; set; }

        [JsonPropertyName("_STATISTICS_WRITING_APP-eng")]
        public string STATISTICSWRITINGAPPEng { get; set; }

        [JsonPropertyName("_STATISTICS_WRITING_DATE_UTC-eng")]
        public string STATISTICSWRITINGDATEUTCEng { get; set; }

        [JsonPropertyName("_STATISTICS_TAGS-eng")]
    ...
    public class MetaInfoModel
    {
        ...
        {
            var splice = format.duration.Split('.');
            return long.Parse(splice[0]) * 1000 + int.Parse(splice[1].Replace("0", ""));
        }

        //public static async Task<long> Do(ProcessIt p, string filepath)
        //{
        //    var info = await Direct.VideoMetaInfo(p, filepath);
        //    var splice = info.format.duration.Split('.');
        //    var cudi = int.Parse(splice[0]) * 1000 + int.Parse(splice[1].Replace("0", ""));
        //    return cudi;
        //}
    }
}
